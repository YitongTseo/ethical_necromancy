...
class FaceEmotionDetection:
    ...
    def findFaces(
        ...
        if self.imgRGB is not None:
            result, emotions, bbox = self.get_dominant_emotion(
                self.imgRGB, detector_backend
            )
            ...
            if draw:
                if fancyDraw:
                    ...
                    cv2.rectangle(
                        img,
                        (bbox[0], bbox[1]),
                        (bbox[0] + bbox[3], bbox[2] + bbox[4]),
                        (255, 0, 255),
                        2,
                    )
                self.draw_text_with_background(
                    img, result.title(), bbox, (255, 255, 255), self.bg_color[1]
                )
...
def MonitorEmotion_From_Video(video_path: int, output_path: str) -> None:
    """
