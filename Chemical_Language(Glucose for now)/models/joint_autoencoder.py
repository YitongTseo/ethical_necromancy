...
class JointAutoencoder(pl.LightningModule):
    ...
    def forward(
        ...
        word_reconstruction = self.word_decoder(word_latent, word_input_ids, word_tgt_mask)
    ...
    def training_step(self, batch, batch_idx):
        ...
        word_input_ids_one_hot = F.one_hot(word_input_ids.squeeze(), num_classes=vocab_size).float()

        word_input_ids_one_hot = word_input_ids_one_hot.contiguous().view(-1, vocab_size)  # [batch_size * max_len, vocab_size]
        word_reconstruction = word_reconstruction.contiguous().view(-1, vocab_size)  # [batch_size * max_len, vocab_size]
        # word_reconstruction.argmax(dim=-1).shape
        # word_input_ids_one_hot.argmax(dim=-1).shape

        word_loss = self.word_reconstruction_criterion(word_reconstruction, word_input_ids_one_hot)
