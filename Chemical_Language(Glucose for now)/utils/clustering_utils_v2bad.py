...
def get_ds2_clusters(
    dataset_2, percentile, further_split_percentile, min_cluster_num, min_cluster_size
):
    ...
    ds2_clusters = divisive_clustering(dataset_2, percentile, min_cluster_size)
...
def get_divisive_clusters(dataset_1, dataset_2, depth=None, ds1_percentile=10):
    ...
    # TODO: this will be gross, but maybe we want to make this a hyperopt sweep?
    # TODO: honestly fuck it let's do it... like 10 samples.
    ds2_clusters = get_ds2_clusters(
        dataset_2,
        percentile=ds1_percentile / 10,
        further_split_percentile=ds1_percentile / 100,
        min_cluster_num=num_ds1_clusters,
        min_cluster_size=max_ds1_cluster_size,
    )
    return ds1_clusters, ds2_clusters


def get_kmeans_cluster(dataset_1, dataset_2, depth, initial_num_clusters=20):
...
# _ds1_clusters & _ds2_clusters arguments are just included for making the boilerplate work
def hungarian_mapping(dataset_1, dataset_2, _ds1_clusters=None, _ds2_clusters=None):
...
def size_mapping(_datset_1, _dataset_2, ds1_clusters=None, ds2_clusters=None):
    """
...
def recursive_grouping_and_clustering(
    dataset_1,
    dataset_2,
    depth=0,
    max_depth=5,
    clustering_method=get_divisive_clusters,
    mapping_method=hungarian_mapping,
    bottom_out_size=10,
    debug=False,
):
    if len(dataset_1) > len(dataset_2):
        print(f"WARNING! len(dataset_1) {len(dataset_1)} > len(dataset_2) {len(dataset_2)}")
    if (
        len(dataset_1) <= bottom_out_size
        or len(dataset_2) <= bottom_out_size
        or depth >= max_depth
    ):
        # Lowest level always default to hungarian_mapping
        return None, None, hungarian_mapping(dataset_1, dataset_2)

    ds1_clusters, ds2_clusters = clustering_method(dataset_1, dataset_2, depth=depth)
    ...
    cluster_mapping = mapping_method(
        ds1_centroids, ds2_centroids, ds1_clusters, ds2_clusters
    )
    ...
    for ds1_cluster, ds2_cluster in iterator:
        ...
        _, _, sub_mapping = recursive_grouping_and_clustering(
            ds1_cluster_points, ds2_cluster_points, depth + 1, max_depth
        )
